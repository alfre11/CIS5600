...
void MyGL::assignPostProcessShaderUniforms() {
    ...
    // Set the current time (u_Time) for time-based effects
    if (selectedPostProcessShader->isUniformHandleValid("u_Time")) {
        GLuint timeHandle = selectedPostProcessShader->getUniformHandle("u_Time");
        glContext.glUniform1f(timeHandle, currTime);
    }

    // Set the texture to use for post-processing (if applicable)
    if (selectedPostProcessShader->isUniformHandleValid("u_Texture")) {
        GLuint textureHandle = selectedPostProcessShader->getUniformHandle("u_Texture");
        glContext.glUniform1i(textureHandle, 0);
    }
