...
void Polygon::Triangulate()
{
    ...
    int n = m_verts.size() - 2;
    for (int i = 0; i < n; i++)
    {
        Triangle nextTri;
        nextTri.m_indices[0] = 0;
        nextTri.m_indices[0] = i + 1;
        nextTri.m_indices[0] = i + 2;
        m_tris.push_back(nextTri);
    }
...
//function to find a bounding box of triangle
void Polygon::getBoundBox(Triangle t, float* xMin, float* yMin, float* xMax, float* yMax) const {
    float xMin1 = std::min(m_verts[t.m_indices[0]].m_pos[0], m_verts[t.m_indices[1]].m_pos[0]);
    *xMin = std::min(xMin1, m_verts[t.m_indices[2]].m_pos[0]);

    float yMin1 = std::min(m_verts[t.m_indices[0]].m_pos[1], m_verts[t.m_indices[1]].m_pos[1]);
    *yMin = std::min(yMin1, m_verts[t.m_indices[2]].m_pos[1]);

    float xMax1 = std::max(m_verts[t.m_indices[0]].m_pos[0], m_verts[t.m_indices[1]].m_pos[0]);
    *xMax = std::max(xMax1, m_verts[t.m_indices[2]].m_pos[0]);

    float yMax1 = std::max(m_verts[t.m_indices[0]].m_pos[1], m_verts[t.m_indices[1]].m_pos[1]);
    *yMax = std::max(yMax1, m_verts[t.m_indices[2]].m_pos[1]);
}
