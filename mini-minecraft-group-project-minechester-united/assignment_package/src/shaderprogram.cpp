...
void ShaderProgram::parseShaderSourceForVariables(char *vertSource, char *fragSource) {
    ...
    for(QString &line : vs_list) {
        ...
        if(sub[0] == "in") {
            ...
            // std::cout << "in added: " << sub[2].left(sub[2].indexOf(";")).toStdString().c_str() << std::endl;
        }
    }
...
void ShaderProgram::drawOverlay(Drawable &d) {
    ...
    int handle;
    if ((handle = m_attribs["vs_Pos"]) != -1 && d.bindBuffer(POSWATER)) {
        context->glEnableVertexAttribArray(handle);
        context->glVertexAttribPointer(handle, 4, GL_FLOAT, false, 0, nullptr);
    }
    ...
    if ((handle = m_attribs["vs_UV"]) != -1 && d.bindBuffer(UVWATER)) {
        // qDebug() << "vs_uv" << handle;
        context->glEnableVertexAttribArray(handle);
...
void ShaderProgram::drawInterleaved(Drawable &d, bool transparent) {

    context->printGLErrorLog();
    if(!transparent && d.elemCount(INDEX) < 0) {
        throw std::invalid_argument(
            "Attempting to draw a Drawable that has not initialized its count variable! Remember to set it to the length of your index array in create()."
            );
    } else if(d.elemCount(INDEXTRANSP) < 0) {
        throw std::invalid_argument(
            "Attempting to draw a Drawable that has not initialized its count variable! Remember to set it to the length of your index array in create()."
            );
    }
    ...
    BufferType inter;
    BufferType interIndex;

    if(transparent) {
        inter = INTERLEAVEDTRANS;
        interIndex = INDEXTRANSP;
    } else {
        inter = INTERLEAVEDOPAQUE;
        interIndex = INDEX;
    }

    int handle;
    if (d.bindBuffer(inter)) {
        ...
        // std::cout << m_attribs["vs_UV"] << std::endl;

        if ((handle = m_attribs["vs_UV"]) != -1) {
            // std::cout << "handle: " << handle << std::endl;d
            context->glEnableVertexAttribArray(handle);
            context->glVertexAttribPointer(handle, 4, GL_FLOAT, false, 3 * sizeof(glm::vec4), (void*) (2 * sizeof(glm::vec4)));
        }
    }

    d.bindBuffer(interIndex);
    // std::cout << d.elemCount(interIndex) << std::endl;
    context->glDrawElements(d.drawMode(), d.elemCount(interIndex), GL_UNSIGNED_INT, 0);
